...
it("explains scope failure and retries successfully", async () => {
  vi.mocked(searchScopeTargetsAction).mockRejectedValueOnce(new Error("offline")).mockResolvedValueOnce([{ id: "c1", label: "Recovered course" }]);
...
it("does not expose superseded query choices", async () => {
  ...
  vi.mocked(searchScopeTargetsAction).mockImplementationOnce(() => new Promise(resolve => { finish = resolve; })).mockResolvedValueOnce([{ id: "new", label: "New target" }]);
  render(<StaffAccountForm roles={[]} />);
  fireEvent.change(screen.getByLabelText("Scope"), { target: { value: "COURSE" } });
  ...
  await act(async () => finish([{ id: "old", label: "Old target" }]));
