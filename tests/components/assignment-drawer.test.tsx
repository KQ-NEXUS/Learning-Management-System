...
it("recovers rejected scope search through retry without stale choices", async () => {
  vi.mocked(searchScopeTargetsAction).mockRejectedValueOnce(new Error("offline")).mockResolvedValueOnce([{ id: "c1", label: "Current course" }]);
...
it("ignores late scope results after the scope changes", async () => {
  let finish!: (value: { id: string; label: string }[]) => void;
  vi.mocked(searchScopeTargetsAction).mockImplementationOnce(() => new Promise(resolve => { finish = resolve; })).mockResolvedValueOnce([{ id: "p1", label: "Current programme" }]);
  openDrawer(); fireEvent.change(screen.getByLabelText("Scope"), { target: { value: "COURSE" } });
  fireEvent.change(screen.getByLabelText("Scope"), { target: { value: "PROGRAMME" } });
  await screen.findByRole("option", { name: "Current programme" });
  await act(async () => finish([{ id: "old", label: "Stale course" }]));
