...
export async function GET(
  ...
  try {
    ...
    // A missing `storageKey` is treated identically to "not found" — never a
    // 500 — since a certificate row can exist without a rendered object.
    if (!certificate || !certificate.storageKey) {
      return new NextResponse(null, { status: 404 });
    }

    const url = await presignCertificateObjectUrl({ key: certificate.storageKey });
